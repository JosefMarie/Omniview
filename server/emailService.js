...
<head>
  ...
  <title>Your OmniView Ticket</title>
</head>
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="background:#131022;border-radius:20px;overflow:hidden;border:1px solid rgba(255,255,255,0.08);">
        ...
        <tr>
          <td style="background:linear-gradient(135deg,#3211d4,#7b5ea7);padding:32px 40px;text-align:center;">
            <p style="margin:0 0 8px;font-size:13px;letter-spacing:0.15em;text-transform:uppercase;opacity:0.8;">OmniView Cinema</p>
        ...
        <tr>
          <td style="padding:20px 40px;border-top:1px solid rgba(255,255,255,0.06);text-align:center;">
            <p style="margin:0;font-size:12px;color:#5a5670;">© ${new Date().getFullYear()} OmniView Cinema · All rights reserved</p>
...
const sendBookingConfirmation = async (order) => {
    ...
    try {
        ...
        const info = await t.sendMail({
            from: `"OmniView Cinema" <${process.env.EMAIL_FROM || process.env.EMAIL_USER || 'noreply@omniview.cinema'}>`,
            to: order.userEmail,
            subject: `🎬 Your OmniView ticket for ${order.movie} is confirmed!`,
