...
export default function RewardsPage() {
    ...
    if (loading) {
        return (
            <div style={{ padding: '8rem 0', textAlign: 'center', color: '#e11d48' }}>
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '4rem 1.5rem' }}>
            ...
            <div style={{ marginBottom: '4rem', textAlign: 'center' }}>
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.6rem', background: 'rgba(225, 29, 72, 0.15)', border: '1px solid rgba(225, 29, 72, 0.3)', borderRadius: '999px', padding: '0.5rem 1.25rem', marginBottom: '1.5rem' }}>
                    <span className="material-symbols-outlined" style={{ color: '#e11d48', fontSize: '1.2rem' }}>stars</span>
                    <span style={{ fontSize: '0.85rem', fontWeight: 900, color: '#e11d48', textTransform: 'uppercase', letterSpacing: '0.15em' }}>Loyalty Program</span>
                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '3rem' }}>
                ...
                <div>
                    {/* Status Card */}
                    <div className="reward-card" style={{ background: 'linear-gradient(135deg, #1e0202 0%, #120101 100%)', border: '2px solid rgba(225, 29, 72, 0.2)', borderRadius: '2.5rem', padding: '3rem', marginBottom: '3rem', position: 'relative', overflow: 'hidden', boxShadow: '0 40px 100px rgba(0,0,0,0.5)' }}>
                        <div style={{ position: 'absolute', top: '-60px', right: '-60px', width: '250px', height: '250px', background: `radial-gradient(circle, ${TIER_COLORS[rewards.tier]}15 0%, transparent 70%)` }} />
                        ...
                        <div style={{ position: 'relative', zIndex: 1 }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '3rem' }}>
                                <div>
                                    ...
                                    <h2 style={{ fontSize: '5rem', fontWeight: 900, color: '#ffffff', lineHeight: 0.9, fontFamily: 'var(--font-display)' }}>{rewards.points} <span style={{ fontSize: '1.5rem', color: '#e11d48', verticalAlign: 'middle', fontWeight: 950 }}>PTS</span></h2>
                                </div>
                            ...
                            {nextTierGoal && (
                                <div style={{ marginBottom: '3rem' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '1rem', marginBottom: '1rem', fontWeight: 800 }}>
                                        ...
                                        <span style={{ color: '#e11d48' }}>{rewards.lifetimePoints} / {nextTierGoal} PTS</span>
                                    </div>
                                    <div style={{ height: '12px', background: 'rgba(255,255,255,0.05)', borderRadius: '6px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.1)' }}>
                                        <div style={{ height: '100%', background: 'linear-gradient(90deg, #e11d48, #be123c)', borderRadius: '6px', animation: 'progressGrow 1.5s cubic-bezier(0.4, 0, 0.2, 1) forwards' }} />
                                    </div>
                                </div>
                            )}
                        ...
                        {Object.entries(TIER_BENEFITS).map(([tier, perks]) => (
                            <div key={tier} style={{ background: 'rgba(255,255,255,0.03)', border: `2px solid ${rewards.tier === tier ? '#e11d48' : 'rgba(255,255,255,0.08)'}`, borderRadius: '2rem', padding: '2rem', opacity: rewards.tier === tier ? 1 : 0.4, transition: 'all 0.3s' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
                                    ...
                                    {rewards.tier === tier && <span style={{ marginLeft: 'auto', fontSize: '0.75rem', fontWeight: 900, background: '#e11d48', color: 'white', padding: '0.4rem 0.8rem', borderRadius: '0.6rem', letterSpacing: '0.05em' }}>ACTIVE</span>}
                                </div>
                                <ul style={{ listStyle: 'none', padding: 0, display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                                    {perks.map((perk, i) => (
                                        <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem', fontSize: '0.95rem', color: 'rgba(255,255,255,0.5)', fontWeight: 600 }}>
                                            <span className="material-symbols-outlined" style={{ fontSize: '1.25rem', color: '#e11d48', fontWeight: 900 }}>check</span>
                                            {perk}
                                        </li>
                                    ))}
                                </ul>
