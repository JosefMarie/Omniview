...
export default function AdminDashboard() {
    ...
    const exportToCSV = () => {
        ...
        const link = document.createElement("a"); link.setAttribute("href", encodeURI(csvContent)); link.setAttribute("download", `Omniview_Report_${new Date().toISOString().slice(0,10)}.csv`); document.body.appendChild(link); link.click(); toast.success('Report downloaded');
    };
        ...
        <div style={{ display: 'flex', height: '100vh', background: '#120101', overflow: 'hidden', color: 'white' }}>
            <aside style={{ width: sidebarCollapsed ? '80px' : '260px', flexShrink: 0, background: '#1e0202', borderRight: '1px solid rgba(225, 29, 72, 0.15)', display: 'flex', flexDirection: 'column', transition: 'width 0.3s cubic-bezier(0.4, 0, 0.2, 1)', overflow: 'hidden' }}>
                <div style={{ padding: '1.5rem', borderBottom: '1px solid rgba(225, 29, 72, 0.15)', display: 'flex', alignItems: 'center', gap: '1rem', height: '72px' }}>
                    ...
                    {!sidebarCollapsed && <span style={{ fontWeight: 900, fontSize: '1.1rem', whiteSpace: 'nowrap', fontFamily: 'var(--font-display)', letterSpacing: '0.02em' }}>OMNIVIEW <span style={{ color: '#e11d48' }}>HQ</span></span>}
                </div>
                    ...
                    {activeNav === 'Settings' && (
                        <div style={{ animation: 'fadeIn 0.5s ease', maxWidth: '700px' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', background: '#1e0202', padding: '2.5rem', borderRadius: '2rem', border: '1px solid rgba(225, 29, 72, 0.15)', boxShadow: '0 30px 60px rgba(0,0,0,0.4)' }}>
                                <div><label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 900, color: '#e11d48', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.75rem' }}>Cinema Brand Name</label><input className="form-input" defaultValue="OmniView Premium Cinema" /></div>
